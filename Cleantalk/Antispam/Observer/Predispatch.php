...
class Predispatch implements ObserverInterface
{
    ...
    public function getConfigValue($settings)
    {
        return $this->scopeConfig->getValue(
            'general/cleantalkantispam/' . $settings,
        ...
        if ( count($arr) ) {
            foreach ( $arr as $key => $value ) {
                ...
                if ( !is_array($value) && !is_object($value) ) {
                    if ( in_array($key, $skip_params, true) && $key != 0 && $key != '' || preg_match(
                            "/^ct_checkjs/",
                            $key
                        ) ) {
                        $contact = false;
    ...
    private function CheckSpam($arEntity)
    {
        ...
        $type = $arEntity['type'];
        if ( $type != 'feedback_general_contact_form' && $type != 'register' ) {
            return;
        }
        ...
        $ct_request->agent = 'magento2-1.6.0';
        ...
        switch ( $type ) {
            case 'feedback_general_contact_form':
                ...
                if ( isset($arEntity['message_title']) ) {
                    $message_title = is_array($arEntity['message_title']) ? implode(
                        " ",
                ...
                if ( isset($arEntity['message_body']) ) {
                    $message_body = is_array($arEntity['message_body']) ? implode(
                        " ",
